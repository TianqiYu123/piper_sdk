...
def cost_function(q, model, data, target_pose):
    ...
    target_pose_matrix = target_pose.as_matrix()
    #print("Current pose",current_pose)
    #print("Target pose",target_pose_matrix)

    # Calculate position error
    position_error = np.linalg.norm(current_pose[:3, 3] - target_pose_matrix[:3, 3])

    # Calculate orientation error (using rotation matrix difference)
    rotation_error = np.linalg.norm(current_pose[:3, :3] - target_pose_matrix[:3, :3])
...
def inverse_kinematics(model, data, target_pose, q_init=None):
    ...
    result = minimize(cost_function, q_init, args=(model, data, target_pose), method='SLSQP', bounds=bounds, tol=1e-6) #tol is tolerance
    elapsed_time = time.time() - start_time
    if result.success:
...
def main():
    ...
    #print("Available frame names:")
    #for i in range(model.nframes):
    #    print(f"Frame ID: {i}, Name: {model.frames[i].name}")  #frame name
    ...
    # Load the robot model into MeshCat
    visual_model = model
    visual_data = visual_model.createData()
    color = np.array([1.0, 0.0, 0.0, 1.0])  # RGBA (Red)
    pin.visualize.MeshcatVisualizer(model=visual_model, collision_model=model, visual_model=model, data=visual_data).initViewer(vis)
    pin.visualize.MeshcatVisualizer(model=visual_model, collision_model=model, visual_model=model, data=visual_data).loadViewerModel(color=color)
        ...
        while True:
            ...
            if endpose_delta is not None and trigger is not None:
                ...
                if trigger == 0:
                    ...
                    #print("Current end pose:", current_end_pose)
                    #IK to find the joint angles, send angles to robot directly
                    
                    #Create target pose as a pinocchio SE3 object
                    target_pose = pin.SE3(Rotation.from_euler('xyz', current_end_pose[3:]).as_matrix(), np.array(current_end_pose[:3]))
                    ...
                    if success:
                        ...
                        # Update the robot configuration in MeshCat
                        q = np.array(joint_angles)  # Convert joint_angles list to a NumPy array
                        pin.forwardKinematics(model, data, q)
                        pin.updateFramePlacements(model, data)

                        pin.visualize.MeshcatVisualizer(model=visual_model, collision_model=model, visual_model=model, data=visual_data).display(q)
